...
class Command(BaseCommand):
    help = "Seed Destination.hero_image from media/mains/*/*-hero.*"
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--overwrite",
            action="store_true",
            help="Replace existing hero image if already set.",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args, **opts):
        ...
        root_dir = (project_root / opts.get("root", "media/mains")).resolve()
        ...
        for sub in sorted(p for p in root_dir.iterdir() if p.is_dir()):
            ...
            # Only look for hero images in this folder
            hero_path = find_one([*sub.glob("*-hero.*")])

            # Guard extensions for hero
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Done. Folders scanned: {total_seen}. Destinations updated: {total_updated}."
        ))
